...
def plot_realized_and_predicted_timeseries(realized: pd.Series = None, expected: pd.Series = None,
    ...
    if isinstance(lower_uncertainty, pd.Series) and len(lower_uncertainty) > 0 and isinstance(upper_uncertainty, pd.Series) and len(upper_uncertainty) > 0:
        ...
        fig.add_trace(go.Scatter(x=lower_uncertainty.index, y=lower_uncertainty.values, mode='lines', line_width=0, line_color='orange', showlegend=False))
        fig.add_trace(go.Scatter(x=upper_uncertainty.index, y=upper_uncertainty.values, mode='lines', line_width=0, line_color='orange', fill='tonexty', name='Uncertainty'))
    ...
    if isinstance(expected, pd.Series) and len(expected) > 0:
        ...
        fig.add_trace(go.Scatter(x=expected.index, y=expected.values, mode='lines+markers', name='Expected', line_color='red', marker_opacity=marker_opacity))

    if isinstance(realized, pd.Series) and len(realized) > 0:
        fig.add_trace(go.Scatter(x=realized.index, y=realized.values, mode='lines+markers', name='Realized', line_color='blue'))
